...
Chat with Claude Code directly from WeChat — powered by Tencent's official iLink Bot API.
...
```sh
git clone https://github.com/xoyoer/claude-channel-wechat.git
cd claude-channel-wechat
bash install.sh
```

脚本会自动完成：依赖安装 → 配置 `~/.mcp.json` → 微信扫码登录。
...
# 2. 配置 ~/.mcp.json（家目录，不是项目目录）
# 如果文件不存在，新建；如果已存在，在 mcpServers 中添加 wechat 条目
```

`~/.mcp.json` 内容（将路径替换为实际克隆位置）：
...
```json
{
  "mcpServers": {
    "wechat": {
      "command": "bun",
      "args": ["run", "--cwd", "/path/to/claude-channel-wechat", "--shell=bun", "--silent", "start"]
    }
  }
}
```

> **注意**：配置文件是 `~/.mcp.json`（家目录），不是 `~/.claude/.mcp.json`，也不是项目目录下的 `.mcp.json`。
...
- **媒体上传流程** — 读文件 → MD5 + AES-128 key → `getuploadurl` → AES-128-ECB 加密 → CDN POST → `sendmessage`
...
Claude Code 的 channel 协议只有 3 种通知，**没有**中间状态（思考、读文件、工具执行）。这些只在终端里可见。
...
`--channels` 会静默丢弃非白名单 channel 的通知。`server:wechat` 不在白名单上，必须用 `--dangerously-load-development-channels`。
...
`get_qrcode_status` 返回的 `bot_token` 已经是完整格式 `botid@im.bot:hex`，直接用。

**4. 权限回复格式**

必须带 5 位码：`y abcde`。只发 `y` 不匹配正则，会被当成普通消息。
...
The script handles everything: dependency install → `~/.mcp.json` config → WeChat QR login.
...
# 2. Configure ~/.mcp.json (home directory — not the project folder)
# Create it if it doesn't exist; add the wechat entry if it does
```

`~/.mcp.json` contents (replace path with your actual clone location):
...
> **Note:** The file must be `~/.mcp.json` (home directory). Not `~/.claude/.mcp.json`, not `.mcp.json` inside the project folder.
...
- **Media upload** — read file → MD5 + AES-128 key → `getuploadurl` → AES-128-ECB encrypt → CDN POST → `sendmessage`
- **Media download** — CDN GET → AES-128-ECB decrypt → save to `inbox/`
- **Send limit** — text and media must be sent as separate `sendmessage` calls
- **SILK audio** — silk-wasm optional dep; falls back to raw `.silk` on transcode failure
...
The Claude Code channel protocol has **no** intermediate-state notifications (thinking, file reads, tool calls). These are only visible in the terminal.

**Workaround:** Open the `/remote-control` URL printed at startup (`https://claude.ai/code/session_...`) on your phone's browser for a real-time view.

### Known Issues

**1. `--channels` vs `--dangerously-load-development-channels`**

`--channels` silently drops notifications from channels not on the built-in allowlist. `server:wechat` is not on that list — always use `--dangerously-load-development-channels`.
...
`get_qrcode_status` returns `bot_token` already in `botid@im.bot:hex` format — use it directly.

**4. Permission reply format**

Must include the 5-char code: `y abcde`. Just `y` fails the regex and goes to Claude Code as a regular message.
