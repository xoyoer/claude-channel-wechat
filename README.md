...
### Relation to ClawBot (openclaw-weixin)

This plugin migrated all media functionality from the official openclaw-weixin ClawBot plugin:
- AES-128-ECB media upload pipeline
- Image / video / file sending
- SILK → WAV voice transcode
- Error notification relay
- Debug mode

openclaw-weixin is no longer used. This plugin is the sole WeChat bot entry point. When ClawBot releases updates, reference its source for sync:
```
...
### 与 ClawBot (openclaw-weixin) 的关系

本插件从 openclaw-weixin（微信官方 ClawBot 插件）迁移了全部媒体功能：
- 媒体上传管线（AES-128-ECB 加密 + CDN 上传）
- 图片/视频/文件发送
- 语音 SILK→WAV 转码
- 错误通知回传
- Debug 模式

openclaw-weixin 不再使用，本插件是微信 bot 的唯一入口。未来 ClawBot 更新时可参照其源码同步更新：
```
参照源码：~/.openclaw/extensions/openclaw-weixin/src/
